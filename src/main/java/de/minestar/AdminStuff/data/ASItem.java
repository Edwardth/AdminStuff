...
public class ASItem {
    ...
    public static ItemStack getItemStack(String name, byte Data, int amount) {
        ...
        if (!isInteger(name))
            ...
            return new ItemStack(Integer.valueOf(name), amount, Data);
    }
    ...
    public static boolean isValid(int TypeID, byte Data) {
        ...
        if (mat.getId() == Material.WOOL.getId() || mat.getId() == Material.INK_SACK.getId()) {
            return true;
        } else if (mat.getId() == Material.LOG.getId() || mat.getId() == Material.MONSTER_EGGS.getId() || mat.getId() == Material.JUKEBOX.getId() || mat.getId() == Material.LONG_GRASS.getId() || mat.getId() == Material.DEAD_BUSH.getId()) {
            if (Data > 2)
                return false;
            return true;
            ...
            if (Data > 1)
                return false;
            return true;
        } else if (mat.getId() == Material.SAPLING.getId() || mat.getId() == Material.LEAVES.getId()) {
            if (Data > 4)
                return false;
            return true;
        } else if (mat.getId() == Material.SMOOTH_BRICK.getId()) {
            if (Data > 3)
                return false;
            return true;
        } else if (Data == 0) {
